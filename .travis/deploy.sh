...
ssh roberta@bookalo.es "source bin/activate && python src/manage.py collectstatic --noinput && python src/manage.py makemigrations && python src/manage.py migrate && deactivate"
