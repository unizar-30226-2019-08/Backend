...
def get_usuario(nombre_usuario):
	return User.objets.filter(username = nombre_usuario)


def get_coordenadas(nombre_usuario):
	Usuario = User.objectss.filter( username = nombre_usuario )
	return Usuario.latitud,Usuario.longitud


def usuario_eliminar(nombre_usuario):
	try:
...
def fetch_productos_usuario( nombre_usuario):
	Lista_productos = Producto.objects.filter(vendido_por = nombre_usuario )
...
def fetch_usuario_opiniones(int ID_usuario):
	return ValidacionEstrella.objects.filter(usuario_valorado = ID_usuario) 
'''
	Pre: 	ID_usuario corresponde al identificador de un usuario
			registrado en el sistema.
...
def fetch_productos_usuario_venta(int ID_usuario):
	return Producto.objects.filter(Q(vendido_por = nombre_usuario) & Q(estado_producto = 'En venta')).distinct()
